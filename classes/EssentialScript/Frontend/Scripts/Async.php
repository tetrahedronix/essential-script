...
/**
 * Concrete decorator: add support for the Async attribute.
 *
 * @author docwho
 */
class Async extends \EssentialScript\Frontend\Scripts\Decorator {
	/**
	...
	public function feature( $tag, $handle, $src ) {
		...
		if ( $this->script->getHandle() === $handle ) {
			$tag = '<script type="text/javascript" async src="' .
